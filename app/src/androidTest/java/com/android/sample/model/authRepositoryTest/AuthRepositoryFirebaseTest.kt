...
class AuthRepositoryFirebaseTest : FirebaseEmulatedTest() {
  ...
  fun getUserById_returnsUser_whenUserExists() = runBlocking {
    ...
    Firebase.firestore.collection("users").document("doc-alice").set(
      mapOf(
        "id" to "alice-123",
  ...
  fun getUserById_returnsNull_whenUserDoesNotExist() = runBlocking {
    ...
    Firebase.firestore.collection("users").document("doc-bob").set(
      mapOf(
        "id" to "bob-456",
        "displayName" to "Bob"
      )
  ...
  fun getUserById_worksWithManyUsers() = runBlocking {
    ...
    users.document("doc-charlie").set(mapOf("id" to "charlie-001", "displayName" to "Charlie")).await()
  ...
  fun getUserById_handlesMissingOptionalFields() = runBlocking {
    ...
    Firebase.firestore.collection("users").document("doc-erin").set(
      mapOf(
        "id" to "erin-777",
      )
    ).await()
