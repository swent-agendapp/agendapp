...
class OrganizationTestHelper {
   ...
   * 
   * Creates test users with predefined names and emails, creates an organization,
   * adds all users to the organization, and sets it as the selected organization.
   * 
  ...
  suspend fun setupOrganizationWithCustomUsers(
    ...
    val users = userDetails.map { (id, displayName, email) ->
