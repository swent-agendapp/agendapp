...
fun LanguageSelectionScreen(
    ...
    Surface(modifier = Modifier.fillMaxSize().semantics { testTag = LanguageSelectionScreenTestTags.ROOT }) {
        Column(
            ...
            Button(
                onClick = {
                    viewModel.applyLanguage(context)
                    onNavigateBack() // Navigate back after applying
                },
                modifier = Modifier.testTag(LanguageSelectionScreenTestTags.APPLY_BUTTON)
            ) {
                Text(stringResource(R.string.language_apply_button_label))
            }
        }
    }
}